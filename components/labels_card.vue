...
function getLabels(): v2KeyValue[] | undefined {
    const filtered_labels = props.key_values?.filter((kv) => {
        if (kv.variant) {
            [v2KeyValueVariant.KEY_VALUE_VARIANT_LABEL, v2KeyValueVariant.KEY_VALUE_VARIANT_STATIC_LABEL].includes(kv.variant)
        } else {
            false
        }
    })
                    ...
                    <tbody v-if="props.key_values">
                        <tr v-for="label in getLabels()">
                            ...
                            <td class="align-self-end p-1">
                                <span v-if="label.variant === v2KeyValueVariant.KEY_VALUE_VARIANT_STATIC_LABEL" class="status status-green">
                                    <IconCheck :size="24" />
                                </span>
                            </td>
                        </tr>
