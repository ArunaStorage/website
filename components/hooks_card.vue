...
function getLabels(): v2KeyValue[] | undefined {
    return props.key_values?.filter((kv) => {
        if (kv.variant) {
            [v2KeyValueVariant.KEY_VALUE_VARIANT_HOOK, v2KeyValueVariant.KEY_VALUE_VARIANT_HOOK_STATUS].includes(kv.variant)
        } else {
            false
        }
    })
                    ...
                    <tbody v-if="props.key_values">
                        <tr v-for="label in getLabels()">
                            ...
                            <td class="align-self-end p-1">
                                <span v-if="label.variant === v2KeyValueVariant.KEY_VALUE_VARIANT_HOOK_STATUS" class="status status-green">
                                    <IconCheck :size="24" />
                                </span>
                            </td>
                        </tr>
